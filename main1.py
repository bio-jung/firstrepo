# Streamlit을 활용한 MBTI 검사 앱 (10문항 버전)
...
def main():
    ...
    with st.form("mbti_form"):
        ...
        q1 = st.radio("사람들과 함께 있을 때 에너지가 생긴다?", ("예", "아니오"))
        q2 = st.radio("사람들과 대화하는 것이 즐겁다?", ("예", "아니오"))

        st.subheader("인식 방식 (S vs N)")
        q3 = st.radio("사실과 현재에 집중하는 편이다?", ("예", "아니오"))
        q4 = st.radio("추상적 개념이나 미래 가능성에 관심이 많다?", ("예", "아니오"))

        st.subheader("판단 기준 (T vs F)")
        q5 = st.radio("논리적 분석을 통해 결정을 내린다?", ("예", "아니오"))
        q6 = st.radio("다른 사람의 감정을 고려해 결정을 내린다?", ("예", "아니오"))

        st.subheader("생활 양식 (J vs P)")
        q7 = st.radio("계획 세우는 것을 선호한다?", ("예", "아니오"))
        q8 = st.radio("즉흥적인 결정을 선호한다?", ("예", "아니오"))

        st.subheader("추가 심화 질문")
        q9 = st.radio("대화 중 침묵이 불편하지 않다?", ("예", "아니오"))
        q10 = st.radio("일상의 작은 세부사항보다 큰 그림을 중요하게 생각한다?", ("예", "아니오"))
    ...
    if submitted:
        ...
        # 점수 배정
        e_score += 1 if q1 == "예" else 0
        e_score += 1 if q2 == "예" else 0
        i_score += 1 if q1 == "아니오" else 0
        i_score += 1 if q2 == "아니오" else 0

        s_score += 1 if q3 == "예" else 0
        s_score += 1 if q9 == "예" else 0
        n_score += 1 if q4 == "예" else 0
        n_score += 1 if q10 == "예" else 0

        t_score += 1 if q5 == "예" else 0
        f_score += 1 if q6 == "예" else 0

        j_score += 1 if q7 == "예" else 0
        p_score += 1 if q8 == "예" else 0
